...
# 运行提交脚本
python submit_fibonacci.py
```

3. **查看结果**:

```bash
curl http://localhost:8000/status/任务ID
```
...
```python
# 在Python代码中导入示例
from legacy.examples import simple_calculation
...
# 通过调度中心提交
import requests
requests.post("http://localhost:8000/submit", json={"code": code})
```
